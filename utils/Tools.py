...
async def check_voter(mem):
    async with aiohttp.ClientSession(
        ...
        async with session.get(
                f"https://top.gg/api/bots/977512459701194852/check?userId={str(mem)}"
        ) as response:
...
def is_voter():
    ...
    async def predicate(ctx: Context):
        async with aiohttp.ClientSession(
            ...
            async with session.get(
                    f"https://top.gg/api/bots/977512459701194852/check?userId={str(ctx.author.id)}"
            ) as response:
                vote = await response.json()
...
def updateDB1(guildID, data):
    with open("bst.json", "r") as config:
        config = json.load(config)
    config["guilds"][str(guildID)] = data
    newdata = json.dumps(config, indent=4, ensure_ascii=False)
    with open("bst.json", "w") as config:
        config.write(newdata)
