...
class Logging(commands.Cog):
  ...
  async def _msglog(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:no:1158411070608769034> | Your top role should be above my top role.")
    save("msg",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:cx_tick:1158669360223748106> | Message log channel updated to {channel.mention}")
  ...
  async def __msglog(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:cx_tick:1158669360223748106> | Member log channel updated to {channel.mention}")
  ...
  async def ___msglog(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:cx_tick:1158669360223748106> | Server log channel updated to {channel.mention}")
  ...
  async def ___msglog_(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:cx_tick:1158669360223748106> | Channel log channel updated to {channel.mention}")
  ...
  async def __msglog__(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:cx_tick:1158669360223748106> | Role log channel updated to {channel.mention}")
  ...
  async def teccnobetamsglog(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:cx_tick:1158669360223748106> | Mod log channel updated to {channel.mention}")
...
Disables all the log in the server."""
  ...
  async def logallenable(self,ctx,channel:discord.TextChannel):
    ...
    await ctx.send(f"<a:cx_tick:1158669360223748106> | All logs channel are updated to {channel.mention}")
  ...
  async def _dis(self,ctx):
    ...
    if data == {"msg": None, "member": None, "server": None, "channel": None, "role": None, "mod": None}:
      return await ctx.reply("<a:no:1158411070608769034> | No logging is enabled.")
    fl[str(ctx.guild.id)] = {"msg": None, "member": None, "server": None, "channel": None, "role": None, "mod": None}
    with open("loggingbyauth.json", "w") as f:
      json.dump(fl,f,indent=4)
    await ctx.reply("<a:cx_tick:1158669360223748106> | Disabled all the logs for this server.")
