...
class Verification(commands.Cog):
  ...
  async def verify_enable(self, ctx, channel: discord.TextChannel, role: discord.Role):
    ...
    if str(ctx.guild.id) in data:
      ...
      btn = Button(style=discord.ButtonStyle.grey, label="", emoji="<a:green_verify:1167398927486697492>", custom_id="Verify")
      view = View()
      view.add_item(btn)
      embed = discord.Embed(title='Verification', description=f"To Access In **{ctx.guild.name}** Press the <a:green_verify:1167398927486697492> Button Below", color=0x00FFCA)
  ...
  async def verify_config(self, ctx):
    with open("verification.json", "r") as f:
      data = json.load(f)
    embed = discord.Embed(title="Verification Configuration", color=0x00FFCA)
