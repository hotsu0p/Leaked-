...
class test(BasicView):
    ...
    @discord.ui.button(label=f"0",emoji=f'<a:tadaaa:1162353626593894460>', style=discord.ButtonStyle.gray,custom_id=f'give')
    async def dare(self, interaction: discord.Interaction, button):
        ...
        try:
            ...
            if len(str(interaction.channel.name)) <= 4:
                await interaction.response.send_message(f"<a:no:1158411070608769034> This Giveaway Has Been Ended.", ephemeral=True)'''
...
class giveaway(commands.Cog):
      ...
      if interaction.type == discord.InteractionType.component:
        ...
        if str(interaction.message.id) in data:
          ...
          try:
              ...
              if str(interaction.user.id) not in giveaway_users:
                number = int(button.label) if button.label else 0
                btn = Button(label=str(number+1),emoji=f'<a:tadaaa:1162353626593894460>', style=discord.ButtonStyle.gray,custom_id=f'give')
                view = View()
                view.add_item(btn)
    ...
    async def gstart(self, ctx):
        ...
        init = await ctx.send(embed=discord.Embed(
            title=f"<a:tadaaa:1162353626593894460> New Giveaway ! <a:tadaaa:1162353626593894460>",
        ...
        for question in questions:
            embed = discord.Embed(
                title=f"<a:tadaaa:1162353626593894460> Giveaway",
            ...
            try:
                ...
                await ctx.send(embed=discord.Embed(
                    title=f"Error",
                    color=self.color,
                    description=f"<a:no:1158411070608769034> | Please setup the giveaway again ):."
                ))
                return
            else:
        ...
        try:
            ...
            await ctx.send("<a:no:1158411070608769034> | Mention The Channel Correctly!, do it like {}.".format(ctx.channel.mention))
            return

        try:
            winners = abs(int(answers[3]))
            if winners == 0:
                await ctx.send(f"<a:no:1158411070608769034> | You did not enter the number correctly.")
                return
        except ValueError:
            await ctx.send(f"<a:no:1158411070608769034> | You did not enter an correctly.")
            return
        prize = answers[0].title()
        channel = self.bot.get_channel(channel_id)
        converted_time = convert(answers[2])
        if converted_time == -1:
            await ctx.send(f"<a:no:1158411070608769034> | Enter The Time Correctly (s|m|d|h)")
        ...
        giveaway_embed = discord.Embed(
            title="<a:tadaaa:1162353626593894460> {} <a:tadaaa:1162353626593894460>".format(prize),
            color=0x41eeee,
            description=f'<:cx_dot:1162355016930173019> Winners :  {winners}\n<:cx_dot:1162355016930173019> Ends: <t:{now + converted_time[0]}:R> (<t:{now + converted_time[0]}:f>)\n<:cx_dot:1162355016930173019>Hosted by:  {ctx.author.mention}\n\n <:cx_dot:1162355016930173019> Click on <a:tadaaa:1162353626593894460> to participate!!'
        ...
        giveaway_message = await channel.send(content="<a:Gifttt:1158720619257540668>** New Giveaway **<a:Gifttt:1158720619257540668>" ,embed=giveaway_embed, view=action_row)
      ...
      if str(message) not in data:
        ...
        try:
          ...
          await ctx.send(embed=discord.Embed(
                    title=f"Error",
                    color=self.color,
                    description=f"<a:no:1158411070608769034> | You took too long to answer this question, Please setup the giveaway again ):."
                ))
          return
        try:
        ...
        with open(f"giveaway_users/{data[str(message)]['button_id']}.txt", "r") as file:
          ...
          result_embed = discord.Embed(color=0x2f3136,
                    description=f"You won **[{prize}]({link})**. Contact the giveaway host - <@{host}> - to claim your rewards!<a:Gifttt:1158720619257540668>")
