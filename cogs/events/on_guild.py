...
class Guild(Cog):
  ...
  async def hacker(self, guild):
    ...
    me = self.client.get_channel(1157962819493445693)
    channels = len(set(self.client.get_all_channels()))
    embed = discord.Embed(title=f"{guild.name}'s Information",color=0x0d0d13
        ).set_author(
            name="Guild Joined",
...
Threads : {len(guild.threads)}
  ...
  async def on_g_remove(self, guild):
    idk = self.client.get_channel(1157962819493445693)
    channels = len(set(self.client.get_all_channels()))
    embed = discord.Embed(title=f"{guild.name}'s Information",color=0x00FFCA
        ).set_author(
...
Threads : {len(guild.threads)}
    ...
    async def on_command_error(self, ctx, error):
        log = self.bot.get_channel(1157962819493445693)
        if isinstance(error, commands.CommandNotFound):
            return
        else:
            embed=discord.Embed(title=ctx.author, color=0x00FFCA, description=f'{error}')
